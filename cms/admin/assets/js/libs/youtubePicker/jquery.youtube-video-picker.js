(function ($) {
    $.fn.YouTubePicker = function (options) {
        ...
        var defaults = {
			...
			, ControlVideoIdHolder:		'.YouTubePickerIdUrl'
                ...
                if (rfeed.length > 0) {
                    ...
                    $ctrVideoList.find("li a").unbind('click').bind('click', function () {
                        ...
                        try {
							...
							var vidId = _GetYouTubeIdFromUrl(selectedUrl);
							
                            $(options.ControlVideoPlayerHolder + "").html(_GetPlayerHtml(vidId));
                            $(options.ControlVideoIdHolder).attr('value', vidId);
